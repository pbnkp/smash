/* smash web — payload module. Sole author: pbnkp. MIT.
   ...
   byte-compatible with the smash CLI (.smash.txt, self-describing manifest). */
(function(){
"use strict";
var V="5.2", $=function(s,r){return (r||document).querySelector(s)}, CE=function(t,c,x){var e=document.createElement(t); if(c)e.className=c; if(x!=null)e.textContent=x; return e};
...
+".card .st{display:flex;gap:.9rem;margin-top:.35rem;font-family:ui-monospace,'SF Mono',Menlo,monospace;font-size:.72rem;color:var(--dim);font-variant-numeric:tabular-nums}"
+".bar{height:3px;background:var(--srf2);border-radius:2px;margin-top:.55rem;overflow:hidden}"
+".bar i{display:block;height:100%;background:var(--acc)}"
+".card .ac{display:flex;gap:.5rem;margin-top:.65rem}"
...
+"#seal{margin-top:2.2rem;padding-top:.9rem;border-top:1px solid var(--ln);display:flex;align-items:center;gap:.55rem;font-family:ui-monospace,'SF Mono',Menlo,monospace;font-size:.68rem;color:var(--dim)}"
...
+"<div class=sub>files to smash &mdash; or a .b64 artifact to restore</div>"
...
function b64(bytes){var s="",i,c=bytes,CH=32768;for(i=0;i<c.length;i+=CH)s+=String.fromCharCode.apply(null,c.subarray(i,i+CH));return btoa(s)}
...
function fname(s){var x=String(s).split("/").pop().replace(/[^A-Za-z0-9._-]+/g,"-").replace(/[._-]{2,}/g,"-").replace(/^[._-]+|[._-]+$/g,"");return x||"data"}
...
async function pump(stream){var r=stream.getReader(),parts=[],n=0,x;while(!(x=await r.read()).done){parts.push(x.value);n+=x.value.length}var o=new Uint8Array(n),i,off=0;for(i=0;i<parts.length;i++){o.set(parts[i],off);off+=parts[i].length}return o}
function viaStream(bytes,T,kind){var s=new Blob([bytes]).stream().pipeThrough(new T(kind));return pump(s)}

function manifest(label,kind,bytes,sum){
...
/* ---- encode / decode ---- */
async function encodeBytes(label,kind,bytes){
 var sum=await sha(bytes);
 var gz=await viaStream(bytes,CompressionStream,"gzip");
 var art=manifest(label,kind,bytes.length,sum)+b64(gz)+"\n";
 var name=fname(label)+".smash.txt";
 card(name,bytes.length,art.length,art,sum);
 return art;
}
function isArtifact(text){return /^# ==== SMASH ARTIFACT|^[A-Za-z0-9+/=\s]+$/.test(text.trim())&&/\.b64\.|# ==== SMASH ARTIFACT|^\/Td6|^H4sI/m.test(text.trim())}
async function decodeText(name,text){
 var man=parseManifest(text);
 var body=text.split("\n").filter(function(l){return l.charAt(0)!=="#"}).join("").replace(/\s+/g,"");
 var comp;
 if(/\.xz\.b64\.|\.zst\.b64\./.test(name)||/xz\(|zst\(|zstd\(/.test(man.encoding||"")||/^\/Td6/.test(body)){cardErr(name,"xz/zstd artifact — restore with the CLI: smash -d "+sane(name),man);return}
 try{comp=unb64(body)}catch(e){cardErr(name,"not valid base64 — is this a smash artifact?");return}
 try{
  var raw=await viaStream(comp,DecompressionStream,"gzip");
  var base=fname(man.source||name).replace(/\.gz\.b64\..*$/,"").replace(/\.smash(?:\.\d+)?\.txt$/,"")||"restored";
  var sum=await sha(raw);
  cardRestored(base,raw,sum,man);
 }catch(e){cardErr(name,"gzip restore failed — if this is an xz artifact use the CLI",man)}
}
function manBlock(man){
 if(!man||!Object.keys(man).length)return null;
 var order=["source","kind","bytes","sha256","encoding","lossy","created","tool"],seen={},parts=[],i;
...
/* ---- cards ---- */
function actions(el,getBlob,name,artText){
 var ac=CE("div","ac");
 var save=CE("button",null,"SAVE"); save.onclick=function(){var a=CE("a");a.href=URL.createObjectURL(getBlob());a.download=name;a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},4000)}; ac.appendChild(save);
 if(artText!=null){var cp=CE("button",null,"COPY"); cp.onclick=function(){navigator.clipboard.writeText(artText).then(function(){cp.textContent="COPIED";setTimeout(function(){cp.textContent="COPY"},1200)})}; ac.appendChild(cp)}
 if(navigator.share){var sh=CE("button",null,"SHARE"); sh.onclick=function(){var f=new File([getBlob()],name,{type:"text/plain"});(navigator.canShare&&navigator.canShare({files:[f]})?navigator.share({files:[f]}):navigator.share({text:artText||name})).catch(function(){})}; ac.appendChild(sh)}
 el.appendChild(ac);
}
function card(name,inB,outB,artText,sum){
 var c=CE("div","card"),r=Math.min(100,Math.round(outB*100/Math.max(1,inB)));
 c.appendChild(CE("div","nm",name));
 var s=CE("div","st"); s.innerHTML="<span>"+fmt(inB)+" → "+fmt(outB)+"</span><span>"+r+"%</span><span class=ok>sha ✓ "+sum.slice(0,12)+"…</span>"; c.appendChild(s);
 var b=CE("div","bar"),i=CE("i"); i.style.width=r+"%"; b.appendChild(i); c.appendChild(b);
 actions(c,function(){return new Blob([artText],{type:"text/plain"})},name,artText);
 $("#out").prepend(c);
}
function cardRestored(base,raw,sum,man){
 var c=CE("div","card");
 c.appendChild(CE("div","nm","restored → "+base));
 var s=CE("div","st"); s.innerHTML="<span>"+fmt(raw.length)+"</span><span class=ok>sha256 "+sum.slice(0,12)+"…</span>"; c.appendChild(s);
 var mb=manBlock(man); if(mb){ if(man.sha256){var vv=CE("span",null,(man.sha256===sum?" ✓ matches source":man.lossy==="yes"?" (lossy)":" ✗ differs")); vv.className=(man.sha256===sum||man.lossy==="yes")?"ok":""; s.appendChild(vv);} c.appendChild(mb); }
 actions(c,function(){return new Blob([raw])},base,null);
 $("#out").prepend(c);
}
function cardErr(name,msg,man){
 var c=CE("div","card err");
 c.appendChild(CE("div","nm",sane(name)));
 c.appendChild(CE("div","st",msg));
...
/* batch queue with visible progress + cancel. cancelFlag stops the run
   between items (each item is atomic; a running gzip finishes). */
var cancelFlag=false, running=false;
function qShow(done,total){var q=$("#q");q.hidden=false;q.classList.add("on");$("#qtxt").textContent="processing "+done+" / "+total+(cancelFlag?" — cancelling…":"")}
function qHide(){var q=$("#q");q.classList.remove("on");q.hidden=true}
/* manifest viewer: parse the `# ` header lines an artifact carries. */
...
async function handleFiles(list){
 ...
 for(i=0;i<total;i++){
  ...
  try{
   ...
   if(/\.b64\.|\.smash(?:\.\d+)?\.txt$/.test(f.name)){await decodeText(f.name,new TextDecoder().decode(bytes))}
   else{await encodeBytes(f.name,"file",bytes)}
...
async function go(){
 var t=$("#ta").value;
 if(t&&t.trim()){
  if(/# ==== SMASH ARTIFACT/.test(t)||/^\/Td6|^H4sI/m.test(t.trim())){await decodeText("pasted-artifact.smash.txt",t)}
  else{await encodeBytes("Pasted Text.txt","string",new TextEncoder().encode(t))}
  $("#ta").value="";press();return;
 }
 $("#fp").click();
}
$("#plate").onclick=function(){$("#fp").click()};
$("#plate").onkeydown=function(e){if(e.key==="Enter"||e.key===" "){e.preventDefault();$("#fp").click()}};
$("#fp").onchange=function(e){handleFiles(e.target.files)};
$("#go").onclick=go;
$("#cancel").onclick=function(){cancelFlag=true;$("#qtxt").textContent+=" — cancelling…"};
...
["dragleave","drop"].forEach(function(ev){document.addEventListener(ev,function(e){e.preventDefault();if(ev==="drop"&&e.dataTransfer&&e.dataTransfer.files.length)handleFiles(e.dataTransfer.files);$("#plate").classList.remove("over")})});
...
/* Service worker: only over http(s) (never file://). The SW verifies each
   cached asset's SHA-256 against a pinned integrity manifest at install and
   fails closed on mismatch — offline use can't be silently poisoned. */
...
if(location.search.indexOf("smashtest=1")>-1){
 encodeBytes("crosstest.txt","file",new TextEncoder().encode("cross-impl pig 12345\nline two\n")).then(function(art){

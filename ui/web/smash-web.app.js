...
async function encodeBytes(label,kind,bytes){
 ...
 var name=fname(label)+".gz.b64."+ts()+".txt";
...
async function decodeText(name,text){
 ...
 var comp;
 if(/\.xz\.b64\.|\.zst\.b64\./.test(name)||/^\/Td6/.test(body)){cardErr(name,"xz/zstd artifact — restore with the CLI: smash -d "+sane(name),man);return}
 ...
 try{
  ...
  var base=fname(name).replace(/\.gz\.b64\..*$/,"")||"restored";
...
async function handleFiles(list){
 ...
 for(i=0;i<total;i++){
  ...
  try{
   ...
   if(/\.b64\./.test(f.name)){await decodeText(f.name,new TextDecoder().decode(bytes))}
...
async function go(){
 var t=$("#ta").value;
 if(t&&t.trim()){
  if(/# ==== SMASH ARTIFACT/.test(t)||/^\/Td6|^H4sI/m.test(t.trim())){await decodeText("pasted-artifact.gz.b64.txt",t)}
  else{await encodeBytes("string","string",new TextEncoder().encode(t))}
